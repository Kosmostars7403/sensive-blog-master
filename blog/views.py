...
def post_detail(request, slug):
    ...
    tags_prefetch = Prefetch('tags', queryset=Tag.objects.annotate(taged_amount=Count('posts')))
    comments_prefetch = Prefetch('comments', queryset=Comment.objects.prefetch_related('author'))

    post = Post.objects.annotate(Count('likes')).prefetch_related(tags_prefetch,comments_prefetch, 'author').get(slug=slug)
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all().prefetch_related(tags_prefetch).prefetch_related('author')[:20].fetch_with_comments_count()
